...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!article) return { title: "Article not found | Radio Dandana" };
  ...
  return {
    title:       `${article.title} | Radio Dandana`,
    description: typeof article.excerpt === "string" ? article.excerpt : "",
    alternates:  { languages: { en: `/en/news/${slug}`, ar: `/news/${slug}` } },
    openGraph: {
...
export default async function EnArticlePage({ params }: Props) {
  const { slug } = await params;
  const shareUrl = `https://dandanaradio.com/en/news/${slug}`;
